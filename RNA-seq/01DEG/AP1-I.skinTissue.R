...
fpkm=getfpkm('AP1_inhibitor/RNA-skintissue/')
...
col=data.frame(row.names = colnames(fpkm),'external_id'=colnames(fpkm),age2=c('AP1I','Ctl','Ctl','AP1I'))
#head(col,2)
countData=fpkm
colData=col
...
up.oy.gene=unique(rownames(up[['condition_AP1I_vs_Ctl']]))
down.oy.gene=unique(rownames(down[['condition_AP1I_vs_Ctl']]))
...
tt=rnkm(res$condition_AP1I_vs_Ctl)
head(tt,2)
table(grepl('Lnc',tt$gene)|grepl('Gm',tt$gene)|grepl('Rik',tt$gene))
tt2=tt[!(grepl('Lnc',tt$gene)|grepl('Gm',tt$gene)|grepl('Rik',tt$gene)),]
dim(tt2)
write.table(tt2,'agingSkinTissue.filter.rnk',quote = F,row.names = F,col.names = F,sep='\t')
